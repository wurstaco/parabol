...
const Team: TeamResolvers = {
  viewerTeamMember: async (
    ...
    const teamMember = await dataLoader
      .get("teamMembers")
      .loadNonNull(teamMemberId);
    return teamMember;
  },
  isViewerOnTeam: async ({ id: teamId }, _args, { authToken }) =>
  ...
  retroMeetingsCount: async ({ id: teamId }, _args, { dataLoader }) => {
    const meetings = await dataLoader
      ...
      .load(teamId);
    const retroMeetings = meetings.filter(
      (meeting) => meeting.meetingType === "retrospective"
    );
  ...
  lastMetAt: async ({ id: teamId }, _args, { dataLoader }) => {
    ...
    const dates = [
      ...completedMeetings.map(
        (meeting) => new Date(meeting.endedAt || meeting.createdAt)
      ),
  ...
  featureFlag: async ({ id: teamId }, { featureName }, { dataLoader }) => {
    return await dataLoader
      ...
      .load({ ownerId: teamId, featureName });
  },
  ...
  massInvitation: async (
    ...
    const matchingInvitation = invitationTokens.find(
      (token) => token.meetingId === meetingId
    );
    if (matchingInvitation) {
      // if the token is < 5 mins old return it
      const createdAt =
        matchingInvitation.expiration.getTime() -
        Threshold.MASS_INVITATION_TOKEN_LIFESPAN;
    ...
    const massInvitation = {
      ...
      userId: viewerId,
      expiration: new Date(
        Date.now() + Threshold.MASS_INVITATION_TOKEN_LIFESPAN
      ),
    };
