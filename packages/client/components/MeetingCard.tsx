...
const MeetingCard = (props: Props) => {
  ...
  const { label: dateLabel, tooltip: readableNextMeetingDate } =
    useMeetingSeriesDate(meeting);
  ...
  const {
    togglePortal: toggleRecurrenceSettingsModal,
    modalPortal: recurrenceSettingsModal,
  } = useModal({ id: "updateRecurrenceSettingsModal" });
  const {
    togglePortal: toggleEndRecurringMeetingModal,
    modalPortal: endRecurringMeetingModal,
  ...
  if (!team) {
    ...
    datadogRum.addError(
      new Error(`Missing Team on Meeting ${JSON.stringify(errObj)}`)
    );
    return null;
  }
          ...
          {meeting &&
            endRecurringMeetingModal(
              <EndRecurringMeetingModal
                meetingRef={meeting}
                recurrenceRule={
                  isRecurring ? meetingSeries.recurrenceRule : undefined
                }
