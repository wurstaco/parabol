...
const rewriteServiceWorker = () => {
  const skeleton = fs.readFileSync(
    path.join(clientDir, "swSkeleton.js"),
    "utf-8"
  );
...
const rewriteIndexHTML = () => {
  ...
  const skeleton = fs.readFileSync(
    path.join(clientDir, "skeleton.html"),
    "utf8"
  );
  // Hide staging & PPMIs from search engines
  const noindex =
    process.env.HOST === "action.parabol.co"
      ? ""
      : `<meta name="robots" content="noindex"/>`;
  const keys = `<script>window.__ACTION__=${JSON.stringify(
    clientKeys
  )}</script>`;
...
const rewriteMattermostEnv = () => {
  ...
  const skeleton = fs.readFileSync(
    path.join(clientDir, "mattermost-plugin_envSkeleton.js"),
    "utf8"
  );
